...
| エージェント | 役割 | 編集 | 実行 | モデル(既定) | 次へのハンドオフ |
| ------------ | ---- | :--: | :--: | -------------- | ---------------- |
| planner | 設計壁打ち | ✕ | ✕ | 中位(例: Claude Sonnet) | task-splitter |
| task-splitter | タスク分解 | ✕ | ✕ | 軽量(例: GPT-5 mini) | implementer |
| implementer | 実装 | ◯ | ◯ | 中〜上位(例: Claude Sonnet) | reviewer |
| reviewer | ローカルレビュー | ✕ | ◯ | 軽量(例: GPT-5 mini) | — |
